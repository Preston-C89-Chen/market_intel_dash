...
const { cotCols } = require("./futures.data")
const COT_COLS =
...
const filePath = path.join(__dirname, './ctfc.json');

function writeJSON(txtData) {
    fs.writeFile('cftc.json', JSON.stringify(txtData, null, 2), (err) => {
      if (err) {
...
async function createCFTC() {
  const jsonData = await readJson("./cftc.json");
  const now = new Date();
  const dateString = now.toISOString();
  const csvClient = new CSV(jsonData,"./test_cftc.csv")
  const csvRes = await csvClient.jsonToCSV(jsonData,"./test_cftc.csv");
}
...
async function getCBreport() {
  ...
  const adaSum = cbData.reduce((memo, row) => {
    ...
    if (row.Asset === "ADA" && row.Spot_Currency === "USD") {
      console.log(row)
      //memo.total = memo.total + Number(row.Total) || 0;
      count += 1;
      q += row.Quantity_Transacted;
      memo.total = memo.total + Number(row.Quantity_Transacted) || 0;
      memo.avg =  memo.avg + Number(row.Spot_Price) || 0;
...
//     if (data.length > 0) {
//       const cftcData = parseCftcData(data,COT_COLS);
